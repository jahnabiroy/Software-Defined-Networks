...
class LearningSwitch(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        if datapath.id not in self.dpid_switch:
            self.dpid_switch[datapath.id] = datapath
    ...
    def packet_in_handler(self, ev):
        ...
        if eth.ethertype == ether_types.ETH_TYPE_LLDP:
            # self.logger.info("Received LLDP packet. Source: %s, Destination: %s, In Port: %s, Switch ID: %s", src, dst, in_port, dpid)
            return
    ...
    def get_topology_data(self, ev=None):
        ...
        for l in links_list:
            ...
            src_mac = l.src.hw_addr  # Get source port MAC address
            dst_mac = l.dst.hw_addr  # Get destination port MAC address
            # self.logger.info("Link between %s and %s on port %s", src_mac, dst_mac, l.src.port_no)
            self.send_packet(l.src.dpid,l.dst.dpid,l.src.port_no,dst_mac,None)
    ...
    def send_packet(self, src_dpid, dst_dpid, port, mac_address, data):
        import time
        datapath = self.dpid_switch.get(src_dpid,None)
        if not datapath:
            self.logger.error("Datapath not found for dpid: %s", src_dpid)
            return
        ...
        self.link_delay.setdefault(src_dpid,{})
        self.link_delay[src_dpid][dst_dpid] = latency
        # self.logger.info("Packet sent to %s from switch %s on port %s. Latency: %.6f seconds", mac_address, src_dpid, port, latency)
